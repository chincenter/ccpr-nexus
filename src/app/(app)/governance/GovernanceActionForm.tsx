...
import { createGovernanceAction } from "./actions";
...
export function GovernanceActionForm({
  ...
  return (
    <form
      action={(formData) => {
        ...
        startTransition(async () => {
          const result = await createGovernanceAction(formData);
    ...
    >
      <input name="action_description" placeholder="Action" required className="rounded-md border border-slate-300 px-2 py-1.5 text-sm lg:col-span-3" />
      <div>
      ...
      <select name="decision_id" className="rounded-md border border-slate-300 px-2 py-1.5 text-sm">
        <option value="">No linked decision</option>
        {decisions.map((d) => (
          <option key={d.id} value={d.id}>{d.decision_text}</option>
        ))}
      </select>
